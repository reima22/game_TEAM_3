...
void CPlayer::Collision(void)
{
	...
	// ボーナスカウント
	CBonus *pBonus;
	pBonus = CGame::GetBonus();

	// メッシュ
	CTerrain *pTerrain = CGame::GetTerrain();

	// チェックポイントロゴ
	CCheckpointLogo *pCheckPointLogo = CGame::GetCheckPointLogo();
				...
				if (pObject->GetObjAttribute() != CObject::OBJATTRIBUTE_PHANTOM)
				{
					if (pCollision->Collision3D(this, pModel) == true)
					{
						...
						case CObject::OBJATTRIBUTE_SAVE:	// 中間地点
							...
							{
								// 一時的保存処理
								pBonus->GetCount()->SaveBonus(pBonus->GetCount()->GetNumber());
