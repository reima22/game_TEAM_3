...
// コンストラクタ
//==============================================================================
CResultScoreEx::CResultScoreEx(int nPriority) : CResult(nPriority)
{
...
void CResultScoreEx::ShffleNum(int nType)
{
	...
	// イベント終了フラグ
	bool bEndEvent[RESULT_SHFFLE_NUM - 1];
	...
	if (fade == CFade::FADE_NONE)
	{
		if (nType == CResult::EXSCORETYPE_TIMER_CALCU)
		{
			...
			else if (nShuffleCnt < 150 && nShuffleCnt % 2 == 0 && nShuffleCnt >= 30)
			{
				// 0~99999999
		...
		else if (nType == CResult::EXSCORETYPE_COUNT_CALCU)
		{
			...
			else if (nShuffleCnt < 210 && nShuffleCnt % 2 == 0 && nShuffleCnt >= 30)
			{
				// 0~99999999
				m_nScoreEx = (rand() % 10000) * 10000 + (rand() % 10000);
