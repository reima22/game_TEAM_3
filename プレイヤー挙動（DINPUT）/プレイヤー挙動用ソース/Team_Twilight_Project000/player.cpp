...
HRESULT CPlayer::Init(D3DXVECTOR3 pos)
{
	...
	// 影の設定
	m_pShadow = CShadow::Create(m_pos, 3.0f, 3.0f);
...
void CPlayer::Update(void)
{
	...
	// 影の追従
	m_pShadow->SetPosShadow(D3DXVECTOR3(m_pos.x, 0.0f, m_pos.z));
}
...
void CPlayer::UpdateMotion(MOTIONTYPE motionType)
{
	...
	{
		if (m_nFrameCnt >= m_pFrame[motionType][m_nKeyCnt])
		{
			m_nFrameCnt = 0;
