	...
	{ //ファイル展開可能
		...
		while (strcmp("END_SCRIPT", pLoadText) != 0)
		{
			...
			if (strcmp("NUM_MODEL", pLoadText) == 0)
			{
				...
				m_aFileName = new char*[m_nNumModel];
				
				// 2次元配列の確保
				for (int nCnt = 0; nCnt < m_nNumModel; nCnt++)
				{
					m_aFileName[nCnt] = new char[TEXT_NUM];
				}
			}
			...
			{			
				m_pFileModelName[m_nModelCnt] = &m_aFileName[m_nModelCnt][0];

				// モデル名
				fscanf(pFile, "%s %s", &aLoadText[0], &m_aFileName[m_nModelCnt][0]);
			...
			if (strcmp("MOTIONSET", pLoadText) == 0)
			{
				...
				while (strcmp("END_MOTIONSET", pMotionText) != 0)
				{
					...
					if (strcmp("LOOP", pMotionText) == 0)
					{
						...
						m_bLoop[m_nMotionCnt] = (bool)nLoop;
					}
					...
					if (strcmp("NUM_KEY", pMotionText) == 0)
					{
						...
						for (int nCntModel = 0; nCntModel < m_nNumKey[m_nMotionCnt]; nCntModel++)
						{
							...
							{
								// キー要素数だけ確保
								m_pKey[m_nMotionCnt][nCntModel][nCnt] = new float[CPlayer::KEY_MAX];
							}
						}				
					}
							...
							if (strcmp("KEY", pKeyText) == 0)
							{
								...
								while (strcmp("END_KEY", pKeyInfoText) != 0)
								{
									...
									if (strcmp("POS", pKeyInfoText) == 0)
									{
										fscanf(pFile, "%s %f %f %f",
											&aKeyInfoText[0],
											&m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_POS_X],
											&m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_POS_Y],
											&m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_POS_Z]);

										m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_POS_X] += m_pOffset[m_nPartsCnt][CPlayer::KEY_POS_X];
										m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_POS_Y] += m_pOffset[m_nPartsCnt][CPlayer::KEY_POS_Y];
										m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_POS_Z] += m_pOffset[m_nPartsCnt][CPlayer::KEY_POS_Z];
									}												 
									...
									if (strcmp("ROT", pKeyInfoText) == 0)
									{
										fscanf(pFile, "%s %f %f %f",
											&aKeyInfoText[0],
											&m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_ROT_X],
											&m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_ROT_Y],
											&m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_ROT_Z]);

										m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_ROT_X] += m_pOffset[m_nPartsCnt][CPlayer::KEY_ROT_X];
										m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_ROT_Y] += m_pOffset[m_nPartsCnt][CPlayer::KEY_ROT_Y];
										m_pKey[m_nMotionCnt][m_nKeyCnt][m_nPartsCnt][CPlayer::KEY_ROT_Z] += m_pOffset[m_nPartsCnt][CPlayer::KEY_ROT_Z];
									}
								}
...
// データの破棄
//==============================================================================
void CTextDataPlayer::Uninit(void)
{
	// テキストデータの破棄
	...
	// キー情報の破棄
	for (int nCntMotion = 0; nCntMotion < CPlayer::MOTIONTYPE_MAX; nCntMotion++)
	{
		if (m_pKey[nCntMotion] != NULL)
		{
			for (int nCntKey = 0; nCntKey < m_nNumKey[nCntMotion]; nCntKey++)
			{
				for (int nCntParts = 0; nCntParts < CPlayer::KEY_MAX; nCntParts++)
				{
