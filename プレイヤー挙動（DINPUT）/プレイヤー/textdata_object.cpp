...
void CTextDataObject::LoadData(void)
{
	...
	// 各パラメータの動的確保
	m_pType = new int[m_nNumObject + 1];			// 種類
	m_pObjectInfo = new float*[m_nNumObject + 1];	// 位置・角度
	m_pShadowWidth = new float[m_nNumObject + 1];	// 丸影の幅
	m_pShadowDepth = new float[m_nNumObject + 1];	// 丸影の奥行
	  ...
	  // スクリプト読み込み終わりまで
		while (strcmp("END_SCRIPT", pLoadText) != 0)
		{
			...
			if (strcmp("OBJECTSET", pLoadText) == 0)
			{
				...
				// オブジェクト情報の種類だけ確保
				m_pObjectInfo[m_nNumObject] = new float[OBJECTINFO_MAX];
				...
				while (strcmp("END_OBJECTSET", pObjectData) != 0)
				{
					...
					{
						fscanf(pFile, "%s %f %f", &aObjectData[0], &m_pShadowWidth[m_nNumObject], &m_pShadowDepth[m_nNumObject]);
					}
					...
					if (strcmp("POS", pObjectData) == 0)
					{
						fscanf(pFile, "%s %f %f %f", &aObjectData[0], 
							&m_pObjectInfo[m_nNumObject][OBJECTINFO_POS_X],
							&m_pObjectInfo[m_nNumObject][OBJECTINFO_POS_Y],
							&m_pObjectInfo[m_nNumObject][OBJECTINFO_POS_Z]);
					}
					...
					{
						fscanf(pFile, "%s %f %f %f", &aObjectData[0],
							&m_pObjectInfo[m_nNumObject][OBJECTINFO_ROT_X],
							&m_pObjectInfo[m_nNumObject][OBJECTINFO_ROT_Y],
							&m_pObjectInfo[m_nNumObject][OBJECTINFO_ROT_Z]);
					}
				}
